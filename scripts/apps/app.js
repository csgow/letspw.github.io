...
console.log('Зашёл в консоль чтобы выигрышь подкрутить себе?');
console.log('Хуйца соснуть не хочешь?');
console.log('Сьебал с консоли!');
console.log('Хуило блять!');
console.log('Иди лучше ставки ставь :)');

var socketIO = io('ip:2095',{'max reconnection attempts':Infinity});	
...
setTimeout(function() {
  $('.notradelink').addClass('hiddenes');
	...
	$('.inform').removeClass('hiddenes');
		...
		if(getLang() == "ru"){ 	 	if(data.inf1 && _$('inf1')) _$('inf1').innerHTML = getWordNormal(data.inf1,'чел.','чел.','чел.'); } else {	if(data.inf1 && _$('inf1')) _$('inf1').innerHTML = getWordNormal(data.inf1,'user','user','users');  }
  		...
  		socketIO.on('last-winner', function(data){
			...
			var gold = "";
		(data.name.indexOf('OWN-SHOP.TK')>-1)?gold='style="color:gold;"':"";
		var z = data.ava.replace('https','http');
				_$('winner-name').innerHTML = '';
				_$('winner-avatar').innerHTML = ('<a target="_blank" original-title="<p>'+data.name+'</p><p>Его шанс:'+data.chance+'%</p><p>Выиграл:'+data.money+'&nbsp;руб</p>" class="user_title_link" href="/profile.php?steam_id='+data.steamid+'"><img src="' + z + '" width="110" height="110"  alt="1"></a>');
...
var gold = "";
		(data.user.indexOf('OWN-SHOP.TK')>-1)?gold='style="color:gold;"':"";
		
if(getLang() == "ru"){  
 var block					= '<div class="short item_user_steam" id="'+data.steamid+'">';
					block += '<div class="avatar-add left"><a href="http://'+data.sitename+'/profile.php?steam_id='+data.steamid+'" target="_blank"><img src="'+data.ava+'" alt="" title="" /></a></div>';
						block += '<ul>\
					<li><a class="user_title_link" '+gold+' original-title="Перейти в профиль" href="http://'+data.sitename+'/profile.php?steam_id='+data.steamid+'" target="_blank">'+data.user+'</a></li>\
...
$('#items-count-temps').text('0');
			...
			$('.gameend').removeClass('hiddenes');
						...
						setTimeout(function() {
$('.game-point').removeClass('hiddenes');
}, 17000);


setTimeout(function() {
$('.game-point').addClass('hiddenes');
}, 30000);
...
if (sound == 'on') {
		...
		socketIO.on('start-game', function(data){
			$('.gamepause').addClass('hiddenes');
			$('.game-hiddenes').addClass('hiddenes');
			if (sound == 'on') {
...
$('#items-count-temps').text('0');
			...
			$('title').text('OWN-SHOP.TK - Правельная рулетка CSGO!');
			$('#timer').text('00:00');
		});
...
if(getLang() == "ru") row1	= '	<div class="list">\
		...
		$.ajax({
			...
			success : function(message){
				...
				var gold = "";
		(el.winnername.indexOf('OWN-SHOP.TK')>-1)?gold='style="color:gold;"':"";
	...
	var buildHistoryPage = function(history, historyOrder, commission) {
		...
		(el.winnername.indexOf('OWN-SHOP.TK')>-1)?gold='style="color:gold;"':"";
			...
			<div class="top">\
				...
				<div class="number">Игра #' + el.game + '</div><div class="blanks">Хэш раунда: '+ el.hash+' | Число раунда: '+ el.roundnum+'</div>\
			</div>\
